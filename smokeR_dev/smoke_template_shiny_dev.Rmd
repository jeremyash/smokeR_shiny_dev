...
hourly_labels <- hourly_png_files %>%
  stringr::str_extract("[0-9]{10}") %>%   # YYYYMMDDHH
  lubridate::ymd_h() %>%                 # convert to datetime
  format("%H:00 %b %d, %Y")              # 24-hour, hour first

# embed PNGs as base64 so report is self-contained
...
### Hourly Smoke Conditions

Use the slider on the map to view hourly modeled smoke concentrations.

```{r hourly_leaflet_map_show, out.width = "100%", include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}
hourly_leaflet
```
